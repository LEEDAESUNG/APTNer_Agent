...
namespace WindowsFormsApp1
{
    public class Parking
    {
        ...
        {
            _MainForm.HomeLogger("[APTner] VisitCheck SND : 방문예약 확인 요청");
            ...
            try
            {
                if(r["isVisitor"].ToString() ==  "Y")
                {
                    ...
                    _MainForm.HomeLogger("[APTner] VisitCheck RCV : [" + sCarno + "] 방문예약:Y, 동:" + r["dong"].ToString() + ", 호:" + r["ho"].ToString() + ", 방문목적:" + r["purpose"].ToString());
                ...
                {
                    Console.WriteLine(string.Format("{0} : {1}", "방문예약", r["isVisitor"]));  //방문여부
                    _MainForm.HomeLogger("[APTner] VisitCheck RCV : [" + sCarno + "] 방문예약:N");
                }
            }
            catch (Exception e)
            {
                ...
                _MainForm.HomeLogger("[APTner]  VisitCheck RCV : " + e.Message);
            }
        ...
        public void CarIn(string Dong, string Ho, string Carno, string isReg)
        {
            _MainForm.HomeLogger("**********************************************************************************");
            ...
            //post_data = "kaptCode=A10026547&carNo=25구5401&dong=200&ho=200&isResident=Y or N";
            post_data = "kaptCode=" + KaptCode + "&carNo=" + Carno + "&dong=" + Dong + "&ho=" + Ho + "&isResident=" + isReg;
            ...
            {
                if (ex.Message.Contains("400")) // Bad Request(400)
                    _MainForm.HomeLogger("[APTner] CarIn RCV : Bad Request(400)");
                else if (ex.Message.Contains("401")) // Unauthorized Request(401)
                    _MainForm.HomeLogger("[APTner] CarIn RCV : Unauthorized Request(401)");
                else if (ex.Message.Contains("403")) // Forbidden(403)
                    _MainForm.HomeLogger("[APTner] CarIn RCV : Forbidden(403)");
                else if (ex.Message.Contains("404")) // Resource Not Found(404)
                    _MainForm.HomeLogger("[APTner] CarIn RCV : Resource Not Found(404)");
                else if (ex.Message.Contains("405")) // Method Not Allowed(405)
                    _MainForm.HomeLogger("[APTner] CarIn RCV : Method Not Allowed(405)");
                else if (ex.Message.Contains("416")) // Invalid Parameter(416)
                    _MainForm.HomeLogger("[APTner] CarIn RCV : Invalid Parameter(416)");
                else if (ex.Message.Contains("500")) // Internal Server Error(500)
